import type CanvasElement from "../CanvasElement.ts";
import Tool from "./Tool.ts";

export default class Select extends Tool {
  static onMouseDown(cnvs: CanvasElement, data) {
    // mbtn 0 mousedown
    // interact with nodes
    // SELECT TOOL
  ...
  static onMouseUse(cnvs, data) {
    ...
    if (cnvs.pointer.scaling && cnvs.selection.length > 0) {
      ...
      const wStep = stepX;
      const hStep = stepX / ar;

      if (cnvs.canvas.getNodeBlob(node).type === "text/plain") {
      ...
      cnvs.canvas.mutateNode(node, () => {
        return {
          size: [node.size[0], node.size[1]],
        };
      });
    } else if (cnvs.pointer.onNode) {
      for (const selectedNode of cnvs.selection) {
        const node = cnvs.canvas.getNode(selectedNode);
        const posX = node.position[0] + data.delta[0] / cnvs.currentScale;
        const posY = node.position[1] + data.delta[1] / cnvs.currentScale;
