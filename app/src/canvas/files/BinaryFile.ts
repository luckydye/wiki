...
export class BinaryFile {
  static async fetch(path) {
    return fetch(path).then(async (res) => {
      if (res.status === 200) {
        ...
        return new BinaryFile(dataArray);
      } else {
  ...
  static parseBytes(binary, byteOffset, type, littleEndian) {
    ...
    if (type === "char") {
      const byte = BinaryFile.parseBytes(binary, byteOffset, "byte", littleEndian);
      ...
      } else if (BinaryFile.STRUCT[type]) {
        const structData = BinaryFile.unserialize(
          binary,
          byteOffset,
          BinaryFile.STRUCT[type],
          littleEndian,
        );
  ...
  static parseType(binary, byteOffset, type, inputs, littleEndian) {
    if (type[type.length - 1] === "]") {
      ...
      if (Number.isNaN(arrayLength)) {
        ...
        } else if (arrayLength in BinaryFile) {
          arrayLength = BinaryFile[arrayLength];
        } else {
      ...
      for (let i = 0; i < arrayLength; i++) {
        const parsed = BinaryFile.parseType(
          binary,
          byteOffset,
          arrayDataType,
          inputs,
      ...
      if (
        typeMapping[type] &&
        binary.byteLength < byteOffset + typeMapping[type].BYTES_PER_ELEMENT
      ) {
        throw new Error(
          `${type} to ${byteOffset + typeMapping[type].BYTES_PER_ELEMENT} offset out of bounds of ${binary.byteLength - 1}`,
        );
      }

      return BinaryFile.parseBytes(binary, byteOffset, type, littleEndian);
    }
  }
    ...
    for (const key in struct) {
      ...
      if (typeCount > 1) {
        for (let i = 0; i < typeCount; i++) {
          const parsedType = BinaryFile.parseType(
            binary,
            byteOffset,
            type,
            structData,
  ...
  static serializeStruct(types, data) {
    ...
    return BinaryFile.serialize(struct);
  }
    ...
    for (const entry in struct) {
      ...
      for (const key of keys) {
        if (key in typeMapping) {
          ...
          BinaryFile.serialize(value, resultData);
          break;
        }
      }
    }

    return BinaryFile.serializeData(resultData);
  }
  ...
  static serializeData(resultData, littleEndian = true) {
    ...
    for (const data of resultData) {
      ...
      if (typeof value === "string") {
        value = BinaryFile.StringToInt32(value);
      }
